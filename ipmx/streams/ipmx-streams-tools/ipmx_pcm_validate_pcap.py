...
def check_sdp_payload_match(ctx: PcmValidationContext) -> tuple[bool, str] | tuple[bool, str, bool]:
    if ctx.sdp_media is None:
        return untestable("No SDP provided")
    if len(ctx.pcm_report.payload_type_set) != 1:
        return False, f"Observed RTP payload type is not constant: {sorted(ctx.pcm_report.payload_type_set)}"
...
def build_requirements() -> list[Requirement]:
    ...
    add("TR-10-3-7a", "shall",
        "UDP destination port SHALL be even and > 1024 (TR-10-3 §7).",
        lambda c: check_udp_port_even(c.stream_info.dst_port if c.stream_info else None, "TR-10-3 §7"))
    add("TR-10-3-7b", "should",
        "UDP destination port SHOULD be > 5000 (TR-10-3 §7).",
        lambda c: check_udp_port_above_5000(c.stream_info.dst_port if c.stream_info else None, "TR-10-3 §7"))
